...
assert_conditions() {
    ...
    if [ -z "${PROJECT_DIR}" ] || [ -z "${PROJECT_PATH}" ] || [ -z "${OVERLAY_MERGED_FLAG}" ]; then
        echo -e "\t${WARNING} Required environment variables PROJECT_DIR, PROJECT_PATH, and OVERLAY_MERGED_FLAG is not set."
        exit 1
    fi
}
...
install_libretech_wiring_tool() {
    if [ ! -d "${PROJECT_DIR}/libretech-wiring-tool" ]; then
        ...
        echo -e "\t${OK} Installed libretech-wiring-tool"
    else
        echo -e "\t${WARNING} Already installed libretech-wiring-tool."
    fi
