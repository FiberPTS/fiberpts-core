...
assert_conditions() {
    ...
    if [ ! -d "${PROJECT_PATH}/venv" ]; then
        echo "${WARNING_MSG} Virtual environment not found at ${PROJECT_PATH}/venv. Please create it before running this script."
        exit 1
    fi
}
...
install_python_packages() {
    ...
    if [ -f "${py_req_file}" ]; then
        ...
        echo "${WARNING_MSG} Python requirements file not found: ${py_req_file}"
        exit 1
    fi
}
...
main() {
    ...
    install_unix_packages
    install_python_packages
    echo "${OK_MSG} All Python dependencies installed successfully"
}
