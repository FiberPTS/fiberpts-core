...
last_error_timestamp=$(dmesg | grep -E 'panel-ilitek-ili9341.*SPI transfer failed|spi_master.*failed to transfer one message from queue|panel-ilitek-ili9341.*\*ERROR\* Failed to update display' | tail -1 | grep -oP '\[\s*\K\d+\.\d+')

# Initialize the current timestamp and counter
if [ -z "$last_error_timestamp" ]; then
    last_error_timestamp=0
else
    initial_errors=$(dmesg | grep -c -E 'panel-ilitek-ili9341.*SPI transfer failed|spi_master.*failed to transfer one message from queue|panel-ilitek-ili9341.*\*ERROR\* Failed to update display')
...
dmesg --follow | while read line; do
    ...
    if echo "$line" | grep -q -E 'panel-ilitek-ili9341.*SPI transfer failed|spi_master.*failed to transfer one message from queue|panel-ilitek-ili9341.*\*ERROR\* Failed to update display'; then
