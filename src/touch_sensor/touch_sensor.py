...
class TouchSensor:
    ...
    def handle_tap(self) -> bool:
        ...
        is_valid_tap = tap.status == TapStatus.GOOD and order_id is not None and employee_id is not None
        logger.info(f"Tap Status: {tap.status == TapStatus.GOOD}, IDs: {order_id and employee_id and True}")
