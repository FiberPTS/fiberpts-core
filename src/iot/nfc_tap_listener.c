...
void print_log(const char *format, ...) {
  ...
  if (format[0] == '\n') {
    printf("\n%s::read_ultralight::",
           buffer);  // add newline before the timestamp
...
char *get_machine_id() {
  ...
  char *machine_id = malloc(
      33);  // the machine ID is a 32-character string, plus '\0' at the end
...
void print_hex(const uint8_t *pbtData, const size_t szBytes) {
  char message[szBytes * 3 + 1];  // Create a string that can hold all
                                  // hexadecimal values and spaces
  char *current = message;  // Pointer to current position in the message string

  for (size_t szPos = 0; szPos < szBytes; szPos++) {
    current += sprintf(current, "%02x ",
                       pbtData[szPos]);  // Write the hex value to the string
...
NdefMessage parse_ndef_message(unsigned char *data, unsigned int length) {
  ...
  while (i < length) {
    ...
    if (sr) {
      ...
      payload_length = (data[i] << 24) | (data[i + 1] << 16) |
                       (data[i + 2] << 8) | data[i + 3];
      i += 4;
    }
    ...
    NdefRecord record = {mb,        me,   cf,          sr,
                         il,        tnf,  type_length, payload_length,
                         id_length, type, id,          payload};
...
void print_payload_as_ascii(NdefRecord *record) {
  ...
  if (record->TNF == 1 && record->payload_length >= 3 &&
      record->payload[0] == 0x02) {
    // Text record
    char message[record->payload_length + 20];  // Extra space for the initial
                                                // text and the null terminator
    ...
    print_log(message);
  } else if (record->TNF == 1 && record->payload_length >= 1 &&
             record->payload[0] == 0x04) {
    // URI record
    char message[record->payload_length + 20];  // Extra space for the initial
                                                // text and the null terminator
    sprintf(message, "URI (%d bytes): https://www.",
...
void format_payload_as_ascii_without_type(NdefRecord *record,
                                          char *payload_ascii) {
  if (record->payload == NULL) {
  ...
  if (record->TNF == 1 && record->payload_length >= 3 &&
      record->payload[0] == 0x02) {
    // Text record
    strncpy(payload_ascii, (char *)record->payload + 3,
            record->payload_length - 3);
  } else if (record->TNF == 1 && record->payload_length >= 1 &&
             record->payload[0] == 0x04) {
    // URI record
    strcpy(payload_ascii, "https://www.");
    strncat(payload_ascii, (char *)record->payload + 1,
...
void send_nfc_to_airtable(NdefMessage message, char *uid, int uid_length) {
  ...
  char *uid_hex = malloc(15);  // enough space for a 7-byte (14 hex digits)
                               // number, plus null terminator
  ...
  */
  char url[] =
      ...
      "appZUSMwDABUaufib/wflvbbVevox2uQESS/wtrSLYKU4WCoW44kl";
  // Allocate enough space for all payloads combined
  /*
  char* combined_payload = malloc(1024 * message.record_count); // allocate
  enough space for all payloads combined combined_payload[0] = '\0'; //
  Initialize the combined_payload string to be empty
  ...
  NdefRecord* record = &(message.records[i]);
      ...
      strncat(combined_payload, payload_ascii, 1024 * message.record_count);
  // assume each payload is less than 1024 bytes strncat(combined_payload,
  ",", 1024 * message.record_count); // add a comma between each payload
  ...
  while (!interrupted) {
    if (nfc_initiator_select_passive_target(pnd, nmMifare, NULL, 0, &nt) > 0) {
      ...
      for (uint8_t page = 4; page <= num_pages; page++) {
        uint8_t cmd[2] = {
            0x30, page};  // 0x30 is the READ command, then the page number
        size_t cmd_len = sizeof(cmd) / sizeof(uint8_t);
        uint8_t page_buffer[16];
        int res;
        if ((res = nfc_initiator_transceive_bytes(
                 pnd, cmd, cmd_len, page_buffer, sizeof(page_buffer), 0)) < 0) {
          print_log("Error while reading the tag: %s\n",
                    "nfc_initiator_transceive_bytes: RF Transmission "
                    "Error");
          can_read = false;
      ...
      char message_buffer[num_pages * num_bytes * 3 +
                          2];  // Each byte becomes two hexadecimal digits
                               // and a space, plus one for the newline and
      ...
      message_buffer[num_pages * num_bytes * 3 + 1] =
          '\0';  // Add a null terminator at the end

      print_log(message_buffer);
      NdefMessage message =
          parse_ndef_message(data_buffer, sizeof(data_buffer));
      if (can_read) {
      ...
      sleep_interruptible(
          10);  // usleep function takes microseconds, so multiply by 1000
    } else if (same_card) {
